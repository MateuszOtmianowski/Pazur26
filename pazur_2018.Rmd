...
- Podstawową funkcją pakietu jest funkcja map(.x, .f, ...), która gdy .x jest wektorem lub listą wykona funkcję .f dla każdego ich elementu, a przypadku ramki danych dla wykona fukcję .f dla każdej kolumny.
...
## Przykłady użycia map {.top-bottom title-size="small"}
```{r echo = T}
set.seed(1)
...
## Przykłady użycia map, alternatywny zapis {.top-bottom title-size="small"}
```{r echo = T}
...
## map + safely
- map(.x, 'nazwa') wywołuje z każdej listy komponent 'nazwa'
```{r echo = T}
map(l, sqrt_safe) %>% map_dbl('result')
```
- map(.x, n) wywołuje z każdej listy komponent o numerze n
```{r echo = T}
...
## map2 i pmap umożliwiają iteracje po dwóch lub więcej wektorach/listach jednocześnie {.top-bottom title-size="medium"}
